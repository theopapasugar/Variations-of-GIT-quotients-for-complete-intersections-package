...
class Problem:
	...
	def ops_set(monomial_list = monomials(self.dim, self.deg), self.dim, self.hyp_no):
	    """
	...
	def max_semi_dest_sets(onepslist = ops_set(monomials(self.dim, self.deg), self.dim, self.hyp_no), self.dim, self.deg, self.hyp_no):
	    """
	    ...
	    for wall in allwalls:  #generates the de-stabilizing families for each wall
	        set_t = max_sets_t(onepslist, wall, monomial_hypersurface, monomial_hyperplane, self.hyp_no)
	    ...
	    for cham in allchambers:  #generates the de-stabilizing families for each chamber
	        set_t = max_sets_t(onepslist, cham, monomial_hypersurface, monomial_hyperplane, self.hyp_no)
	...
	def max_sets_t(onepslist = ops_set(monomials(self.dim, self.deg), self.dim, self.hyp_no) , t, monomial_list1 = monomials(self.dim, self.deg), monomial_list2 = monomials(self.dim, 1), self.hyp_no):
	    """
	...
	def printout(onepslist = ops_set(monomials(self.dim, self.deg), self.dim, self.hyp_no), self.dim, self.deg, self.hyp_no):
	    """
